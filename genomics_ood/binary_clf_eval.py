...
from genomics_ood import binary_clf
...
def restore_model_from_ckpt(ckpt_dir, ckpt_file):
  ...
  model = binary_clf.SeqPredModel(params)
  model.reset()
...
def main(_):
  ...
  _, ckpt_file = utils.get_ckpt_at_step(FLAGS.model_dir, FLAGS.ckpt_step)
  if not ckpt_file:
    tf.logging.fatal('%s model ckpt not exist', ckpt_file)
  params, model = restore_model_from_ckpt(FLAGS.model_dir, ckpt_file)

  # specify test datasets for eval
  params.in_val_file_pattern = 'in_test'
  params.ood_val_file_pattern = 'ood_test'

  (_, _, in_test_dataset, ood_test_dataset) = binary_clf.load_datasets_and_labels(
      params, mode_eval=True)

  _, _, y_in_test, labels_in_test = model.pred_from_ckpt(
      in_test_dataset, FLAGS.n_samples)
  _, _, y_ood_test, labels_ood_test = model.pred_from_ckpt(
      ood_test_dataset, FLAGS.n_samples)

  ll_test_in = list_to_np(y_in_test[:, 1])
  ll_test_ood = list_to_np(y_ood_test[:, 1])

  labels_in_test = list_to_np(labels_in_test)
  labels_ood_test = list_to_np(labels_ood_test)
  assert np.array_equal(labels_in_test, [0] * len(labels_in_test))
  assert np.array_equal(labels_ood_test, [1] * len(labels_ood_test))
  ...
  # eval for AUC
  auc_clf = compute_auc(ll_test_in, ll_test_ood, pos_label=0)
  print('AUCs for binary classifier: %s' %
        (auc_clf))
