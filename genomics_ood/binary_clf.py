...
def create_out_dir(params):
  ...
  sub_dir = ('bin_classifier_emb%s_l%d_bs%d_lr%.4f_nm%d_lm%d_hd%d'
...
def change_labels(features, label):
  features['y'] = label
  return features
...
def load_datasets_and_labels(params, mode_eval=False):
  ...
  if mode_eval:  # For evaluation, no need to prepare training data
    ...
    in_tr_dataset_p = tf.data.TFRecordDataset(in_tr_data_file_list).map(
  ...
  ood_val_dataset = tf.data.TFRecordDataset(ood_val_data_file_list).map(
      lambda v: change_labels(utils.parse_single_tfexample(v, v), 1))
