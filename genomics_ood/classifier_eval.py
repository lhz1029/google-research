...
from genomics_ood import classifier
...
def restore_model_from_ckpt(ckpt_dir, ckpt_file):
  ...
  model = classifier.SeqPredModel(params)
  model.reset()
...
def main(_):
  ...
  (_, in_test_dataset, ood_test_dataset) = classifier.load_datasets(
      params, mode_eval=True)

  loss_in_test, _, y_in_test, _ = model.pred_from_ckpt(
      in_test_dataset, FLAGS.n_samples)
  loss_ood_test, _, y_ood_test, _ = model.pred_from_ckpt(
      ood_test_dataset, FLAGS.n_samples)

  ll_test_in = -list_to_np(loss_in_test)  # full model likelihood ratio
  ll_test_ood = -list_to_np(loss_ood_test)

  y_test_in = list_to_np(y_in_test)
  y_test_ood = list_to_np(y_ood_test)
  ...
  loss_in_test, _, _, y_in_test = model.pred_from_ckpt_ood_odin(
      in_test_dataset, FLAGS.n_samples)
  loss_ood_test, _, _, y_ood_test = model.pred_from_ckpt_ood_odin(
      ood_test_dataset, FLAGS.n_samples)
  ...
  auc_odin = compute_auc(ll_test_in, ll_test_ood, pos_label=0)

  print('AUCs for classifier (MSP and ODIN): %s, %s ' %
        (auc_clf, auc_odin))
